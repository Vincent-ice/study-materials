...
| EENTRY | 0xC | 例外入口地址 |
| TLBIDX | 0x10 | TLB 索引 |
| TLBEHI | 0x11 | TLB 表项高位 |
| TLBELO0 | 0x12 | TLB 表项低位 0 |
| TLBELO1 | 0x13 | TLB 表项低位 1 |
| ASID | 0x18 | 地址空间标识符 |
| PGDL | 0x19 | 低半地址空间全局目录基址 |
| PGDH | 0x1A | 高半地址空间全局目录基址 |
| PGD  | 0x1B | 全局目录基址 |
| CPUID | 0x20 | 处理器标识 |
...
| TICLR | 0x44 | 定时中断清除 |
| LLBCTL | 0x60 | LLBit控制 |
| TLBRENTRY | 0x88 | TLB重填入口地址 |
| CTAG | 0x98 | Cache Tag |
| DMW0~DMW1 | 0x180~0x181 | 直接映射配置窗口 |
...
| ADEF | 0x8 0x0 | 取指地址错异常 | 指令执行中异常 |
| ADEM | 0x8 0x1 | 访存地址错异常 | 指令执行中异常 |
| ALE | 0x9 | 地址非对齐异常 | 指令执行中异常 |
| BCE | 0xA | 边界约束检查错误异常 | 指令执行中异常 |
| SYS | 0xB | 系统调用异常 | 系统调用和陷入 |
| BRK | 0xC | 断点异常 | 系统调用和陷入 |
| INE | 0xD | 指令不存在异常 | 指令执行中异常 |
| IPE | 0xE | 指令权限等级错异常 | 指令执行中异常 |
